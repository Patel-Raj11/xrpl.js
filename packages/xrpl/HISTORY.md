...
### Added
* Support for `Single Asset Vault` (XLS-65): `VaultCreate`, `VaultSet`, `VaultDelete`, `VaultDeposit`, `VaultWithdraw`, and `VaultClawback` transactions, the `Vault` ledger object, the `vault_info` request, and the `hashVault` utility
